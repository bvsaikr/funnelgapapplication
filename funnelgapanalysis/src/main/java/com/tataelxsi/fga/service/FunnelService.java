			...
			fReq.setReservedEmpNo(currentRow.getCell(map.get("ReservedEmpNo")).getStringCellValue().trim());
				...
				fReq.setReservedNo(currentRow.getCell(map.get("ReservedNo")).getCellType() == org.apache.poi.ss.usermodel.CellType.NUMERIC
                        ...
                        : currentRow.getCell(map.get("ReservedNo")).getStringCellValue().trim() != ""
                                ...
                                : 0);
				 fReq.setRequirementDate(currentRow.getCell(map.get("RequirementDate")).getDateCellValue()); 
				 
              				
				
				
			   
				FunnelCustomer fCust =	this.custrepo.findByCustomerName(currentRow.getCell(map.get("CustomerName")).getStringCellValue().trim());
			     if(fCust == null) {
			    	 
			    	 fCust = new FunnelCustomer();
				...
				FunnelBuc fBuc =this.bucrepo.findByBUNameCompetency(currentRow.getCell(map.get("BUName")).getStringCellValue().trim(),
						currentRow.getCell(map.get("Competency")).getStringCellValue().trim());
					if(fBuc == null) {	
					fBuc = new FunnelBuc();
                ...
                FunnelProject fProj = new FunnelProject();
	             ...
	             FunnelProjectCategory fProCat = this.pcrepo.findByCategory(currentRow.getCell(map.get("Category")).getStringCellValue().trim());
	             if(fProCat == null) {
	            		 
	            	fProCat = new FunnelProjectCategory();
				 ...
				 FunnelProjectProbability fProPro =this.pprepo.findByProbability(currentRow.getCell(map.get("Probability")).getStringCellValue().trim());
						 if(fProPro == null) {
						 
						fProPro =new FunnelProjectProbability();
		        ...
		        fReq.setfProjProb(fProPro);
		         ...
		         FunnelProjectStage fProjStage  = this.psrepo.findByStage(currentRow.getCell(map.get("Stage")).getStringCellValue().trim());
		         if(fProjStage == null) {
		        		 
		        		fProjStage = new FunnelProjectStage();
